...
export default function Watchlist({ onSelect }: { onSelect: (sym: string) => void }) {
  ...
  function addSymbol() {
    ...
    if (!sym) return;
    if (!list.includes(sym)) setList([...list, sym]);
    setInput("");
  }

  function removeSymbol(sym: string) {
    setList(list.filter((x) => x !== sym));
  }
  ...
  return (
    <div style={ui.box}>
      ...
      <div style={ui.addRow}>
        <input
          style={ui.input}
          placeholder="Add symbol (TCS, 540614, BTC)"
          value={input}
      ...
      <div style={ui.list}>
        {list.length === 0 && (
          <div style={{ color: "#7f9ab0", padding: 10 }}>No items yet</div>
        )}
